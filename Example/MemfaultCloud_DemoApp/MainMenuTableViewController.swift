...
class MainMenuTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 0:
            self.getApi().getLatestRelease(for: self.getDeviceInfo()) { (package, isUpToDate, error) in
                if error != nil {
        ...
        case 1:
            ...
            self.getApi().postChunks([chunk1, chunk2], deviceSerial: "DEMO_SERIAL") { (error) in
                if error != nil {
                    self.alert(title: "Error", message: error!.localizedDescription)
                } else {
                    self.alert(title: "OK", message: "Chunks posted!")
                }
            }
        default:
            assert(false)
